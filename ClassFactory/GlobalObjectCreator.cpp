...
void Creator::GlobalObject(std::ofstream& cpp, std::ofstream& h, const std::string& className, const std::string& parentName)
{
	bool inheritance = !parentName.empty();
	/// Preprocess
	std::string parentHeader = parentName;
	if (parentHeader.empty())
		parentHeader = "<DWEngine2D\\GlobalObject.h>";
	else
		parentHeader = '"' + parentHeader + ".h" + '"';

	std::string parentClass = parentName;
	if (parentClass.empty())
		parentClass = "DW::GlobalObject";
	...
	if (inheritance)
	{
		h << "\tDeclare_GlobalObjectDefault(" << className << ")" << std::endl;
	}
	else
	{
		h << "\tDeclare_GlobalObjectDefault(" << className << ")" << std::endl;
	...
	if (inheritance)
		cpp << "Implement_GlobalObjectDefault(" << className << ")" << std::endl;
	else
		cpp << "Implement_GlobalObjectDefault(" << className << ")" << std::endl;
