...
void Creator::AIStateLogicCreator(std::ofstream& cpp, std::ofstream& h, const std::string& className, const std::string& parentName)
{
	bool inheritance = !parentName.empty();
	/// Preprocess
	std::string parentHeader = parentName;
	if (parentHeader.empty())
		parentHeader = "<DWEngine2D\\AIStateLogic.h>";
	else
		parentHeader = '"' + parentHeader + ".h" + '"';

	std::string parentClass = parentName;
	if (parentClass.empty())
		parentClass = "DW::AIStateLogic";
	...
	if (inheritance)
	{
		h << "\tDeclare_AIStateLogicDefault(" << className << ")" << std::endl;
	}
	else
	{
		h << "\tDeclare_AIStateLogicDefault(" << className << ")" << std::endl;
	...
	if (inheritance)
		cpp << "Implement_AIStateLogicDefault(" << className << ")" << std::endl;
	else
		cpp << "Implement_AIStateLogicDefault(" << className << ")" << std::endl;
